	...
	RageUI.DrawContent({header = true, instructionalButton = true}, function()
		...
		RageUI.List(_U('wallet_money_button', ESX.Math.GroupDigits(ESX.PlayerData.money)), PersonalMenu.WalletList, PersonalMenu.WalletIndex['money'] or 1, nil, {}, true, function(Hovered, Active, Selected, Index)
			...
			PersonalMenu.WalletIndex['money'] = Index
                  ...
                  RageUI.Button(_U('Ausweise'), nil, {}, true, function(Hovered, Active, Selected)
				if (Selected) then
				end
			end)
		...
		if Config.JSFourIDCard then
			...
			if ESX.PlayerData.job ~= nil then
				if ESX.PlayerData.job.name == "police" then
					RageUI.Button("Dienstausweis zeigen", nil, {}, true, function(Hovered, Active, Selected)
						if (Selected) then
							local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
					...
					RageUI.Button("Dienstausweis anschauen", nil, {}, true, function(Hovered, Active, Selected)
						if (Selected) then
							TriggerServerEvent('lspd:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()), 'lspd')
						end
					end)
				end
			end
			...
			if ESX.PlayerData.job ~= nil then
				if ESX.PlayerData.job.name == "fib" then
					RageUI.Button("Dienstausweis zeigen", nil, {}, true, function(Hovered, Active, Selected)
						if (Selected) then
							...
							if closestDistance ~= -1 and closestDistance <= 3.0 then
								...
								TriggerServerEvent('fib:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(closestPlayer), 'fib')
							else
								ESX.ShowNotification(_U('players_nearby'))
							end
						end
					...
					RageUI.Button("Dienstausweis anschauen", nil, {}, true, function(Hovered, Active, Selected)
						if (Selected) then
							TriggerServerEvent('fib:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()), 'fib')
						end
					end)
				end
			end
